...
if ( type.equals(DOUBLE) ) {
    ...
    public QDataSet getDataSet(ProgressMonitor mon) throws Exception {
        ...
        int offset = getIntParameter( "byteoffset", 0 );
        ...
        if( dep0==-1 ) {
            ...
            if ( dep0>0 ) defltcol= 0; else defltcol=1;
        }
        
        int col = getIntParameter( "column", defltcol );

        String columnType= getParameter( "type", DOUBLE );

        String encoding= getParameter( "byteOrder", "little" );
        if ( encoding.equals("big") ) buf.order( ByteOrder.BIG_ENDIAN ); else buf.order( ByteOrder.LITTLE_ENDIAN );
        
        MutablePropertyDataSet ds = makeDataSet( 1, length / byteCount(columnType) / fieldCount, fieldCount, col, 1, 1, 0, buf, columnType );

        
        if (dep0 >-1) {
            String dep0Type = getParameter( "dep0Type", columnType );
            
            QDataSet dep0ds = makeDataSet( 1, length / byteCount(columnType) / fieldCount, fieldCount, dep0, 1, 1, 0, buf, dep0Type );
