...
public class FSTreeModel implements TreeModel {
    ...
    class TreeNode {
        ...
        public String toString() {
            if (f.isFile()) {
                return f.getName() + " " + nf.format(f.length() / 1000) + " KB";
            } else {
                Long usages = model.usage(f);
                if (usages == null) {
                    return f.getName() + " ??? KB";
                } else {
                    return f.getName() + " " + nf.format(model.usage(f)) + " KB";
                }
            }
        }
    }
    ...
    public Object getChild(Object parent, int index) {
        File[] ff = listings.get(((TreeNode) parent).getFile());
        return new TreeNode(ff[index]);
    }
